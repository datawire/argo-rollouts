...
func (w *Webhook) SendSetWeightEvent(desiredWeight int32, rollout *v1alpha1.Rollout) {
	...
	if resp.StatusCode != 202 {
		if resp.Body != nil {
			...
			if err != nil {
				log.Warnf("rollout webhook error: code %s: unable to read resp.Body: %s", resp.Status, err)
				return
			}
			defer resp.Body.Close()
			log.Warnf("rollout webhook error: code %s: %s", resp.Status, string(body))
			return
		}
		log.Warnf("rollout webhook error: code %s: no response body", resp.Status)
		return
	}
	log.Infof("rollout webhook sent successfully: code %s", resp.Status)
}
